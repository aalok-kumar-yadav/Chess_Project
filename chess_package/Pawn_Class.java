...
public class Pawn_Class extends Piece_Class{	//Pawn class
	...
	public Pawn_Class(String col, char sym){		//Pawn constructor
		this.color=col;
		this.symbol=sym;
	}
	...
	public boolean move_piece(int x1,int y1,int x2,int y2)
	{
		...
		if((abs1==1&&abs2==0)||(abs1==1&&abs2==1)){ 	//one cell move of pawn
			...
			if((temp_board[x2][y2].symbol==0)&&(abs1==1&&abs2==0)) {
				flag=true;
					
			}else if((abs1==1&&abs2==1)&&(temp_board[x2][y2].color!=board_class_obj.player_color)) {
				flag=true;
			}
		...
		}else if((abs1==2)&&(y2==y1)&&(temp_board[x2][y2].symbol==0)&&(x1==6||x1==1)){  //two cell move of pawn 
			mov_ret=true;
